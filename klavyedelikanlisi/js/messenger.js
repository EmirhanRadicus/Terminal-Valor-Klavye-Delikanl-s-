/* ============================================
   MESSENGER APP — Event-driven NPC system
   Anne + Broker branching story + Paranoyak
   ============================================ */
const MessengerApp = (function () {
  ...
  // Mom's pre-existing chat history (loads on first game)
  const MOM_HISTORY = [
  ...
  // Story branch flag
  let _brokerStoryBranch = null; // 'rebel' or 'obey'

  // ---- EVENT-DRIVEN STORY MESSAGES ----
  const STORY_MESSAGES = {
    // Trigger 1: Broker's first contact — after first successful hack
    brokerFirstContact: {
      contact: 'broker',
      messages: [
      ...
      afterMessages: function () {
        // Show choice buttons after messages
        setTimeout(function () {
          showBrokerFirstChoices();
        }, 9000);
      },
    },
    // Paranoyak VPN warning — first hack at cafe
    paranoyakVpnWarning: {
      contact: 'paranoyak',
      messages: [
    ...
    // Mom periodic messages (triggered on various events)
    momWorry: {
      contact: 'annem',
      messages: [
  ...
  function buildContactList() {
    ...
    Object.values(CONTACTS).forEach(function (c) {
      if (!c.visible) return; // Hide contacts not yet revealed
  ...
  function forceReceiveMessage(senderName, text) {
    ...
    if (senderName === 'Broker') contactId = 'broker';
      ...
      // Unknown contact — create if needed
  ...
  function triggerStoryEvent(eventId) {
    ...
    // Run after-messages callback if exists
  ...
  function showBrokerFirstChoices() {
    // Only show if broker chat is active
    if (_activeContact === 'broker' && WindowManager.isOpen('messenger')) {
      ...
      // Queue the choices for when broker chat is opened
  ...
  function onGameStart() {
    // Initialize mom's chat history if no save exists
    const conversations = GameState.get('conversations');
    ...
    // Trigger mom messages based on time events
