/* ============================================
   CONTEXT MENU — Desktop right-click menu
   ============================================ */
const ContextMenu = (function () {
  ...
  function init() {
    ...
    // Right-click on desktop area (not on windows, taskbar, or icons)
    $('#desktop').on('contextmenu', function (e) {
      // Only trigger on the desktop background itself, not child elements like windows
      ...
      if ($target.closest('.app-window').length ||
        ...
        return; // let the event pass, don't show context menu
      }
    ...
    // Prevent default right-click on #desktop-icons background only
  ...
  function showMenu(x, y) {
    // Update dynamic items based on game state
    updateDynamicItems();
  ...
  function updateDynamicItems() {
    // Nothing dynamic for now, but hook for future
  }
  ...
  function handleAction(action) {
    switch (action) {
      ...
      case 'status':
        Taskbar.openApp('terminal');
        setTimeout(function () {
          // Trigger status command in terminal
